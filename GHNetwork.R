...
geom_bar(stat = "identity", fill = ifelse(actors2$Number_of_contacts > 20, 'red', 'grey')) + 
coord_flip()
# В топ-10 актеров с наибольшим числом контактов входят: Роберт Де Ниро, Том Хэнкс, Аль Пачино, Клинт Иствуд, Брэд Питт,
# Мэтт Деймон, Леонардо Ди Каприо, Кристиан Бэйл, Джеймс Стюарт, Скарлетт Йоханссон.
...
plot(g2, vertex.color= colrs[as.factor(V(g2)$comnew)], edge.curved = 0.2, vertex.size=3.5, vertex.label = NA, 
edge.color = "grey", layout=layout_with_fr) # визуализируем только большие сообщества (с числом вершин от 70 и выше). 
# В данном случае берем такие границы деления, потому что этот алгоритм делит граф на большое число небольших по размеру сообществ (если визуализируем только сообщества размером больше 100 вершин, то получим только одну группу).
# https://github.com/DatExpN/DataAnalysisExample/blob/main/GraphNewman.png

# Дополнительный способ визуализации графа возможен при помощи ggnet2. Для этого потребуется перейти от объекта igraph к объекту network. 
library(intergraph)
library(network)
library(GGally)
...
edge.size = 0.01, mode = 'fruchtermanreingold', size = 2.5) + guides(color = FALSE, size = FALSE) 
# Визуализируем сообщества размером более 100 вершин, полученные после применения алгоритма Louvain.
