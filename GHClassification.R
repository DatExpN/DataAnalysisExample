...
# Основными переменными для анализа в этой базе для нас будут: год релиза (Released_Year), длительность фильма (Runtime), 
...
# В-третьих, для анализа нам понадобятся новые переменные: выручка больше или равная медианному значению, 
