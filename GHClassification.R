...
# Как мы и предполагали, фильмы разделяются по рейтингу: в нашем случае горизонтальное измерение может быть описано при помощи информации об оценках аудитории, а вертикальное измерение через информацию об оценках критиков
ggplot() +
...
ggplot() +
  ...
  annotate(geom = 'label', x = -1, y = 5.5, size = 9.5, label = paste('Stress: ', round(nmds_result$stress, digits = 2)))
# Как мы видим, в случае жанров или года релиза, визуализация не дает нам однозначно предположить, фильмы какого жанра (или какого года релиза) будут выше оценены критиками или аудиторией
...
sapply(dd, class)
fviz_silhouette(pam_fit, palette = "jco", ggtheme = theme_classic()) # проверяем насколько хорошо 5 кластеров описывают наши данные
km_stats <- cluster.stats(gower_dist, pam_fit$clustering)
km_stats$dunn 
dd <- cbind(dd, df[, 6:9])
# Сделаем несколько изображений, чтобы более подробно описать полученные кластеры с точки зрения жанров фильмов, года релиза, выручки и рейтингов. Как мы видим, жанр стал основной переменной, разделивший кластеры.
library("gridExtra")
...
set.seed(3217)
ntree.1 <- 45 # пробуем решения для разного числа деревьев, останавливаемся на том решении, где число ошибок распознавания не меняется
nodesize.1 <-1
keep.forest.1 <- TRUE
